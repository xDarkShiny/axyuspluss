...
member.setNickname(`${tag} ${member.user.username}`)
  ...
  if(db.has(`tagKanal_${member.guild.id}`) === true) {
    if(tagKD === "var") {
      member.guild.channels.get(tagK).send(`<a:onayl:769860723320684575>**${member.user.tag}** adlı kullanıcıya \`${db.fetch(`tagB_${member.guild.id}`)}\` olarak ayarlanmış olan tag verilerek kullanıcının ismi sunucu için \`${member.nickname || `${db.fetch(`tagB_${member.guild.id}`)} ${member.user.username}`}\` olarak ayarlanmıştır!`)
  }}};
  ...
  if (db.has(`sayac_${member.guild.id}`) === true) {
    ...
    const embed = new Discord.RichEmbed()
     .setColor('RANDOM')
     .setDescription(`Gold üye belirdi. \`${member.user.tag}\`, Ayağa kalkın \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.size}\` Kişi Kaldı.`)
    ...
    member.guild.channels.get(channel).send(`<a:onayl:769860723320684575> <a:giri:769860432013164574>**${member.user.tag}** Sunucuya katıldı! \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.size}\` üye kaldı!`)
    }};
