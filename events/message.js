...
if (db.has(`prefix_${message.guild.id}`) === false) {
  ...
  if(db.fetch(`yasakK_${message.guild.id}`).includes(cmd.name)) return message.channel.send('Bu komut bu sunucuda **yasaklanmıştır!**')
  
}
  let veri = await db.fetch(`botbakım`)
if(veri) {
 if(message.author.id !== "543436148056260608") {
 
 let codeming = new Discord.RichEmbed()
 .setTitle(`Bakımdayız`)
 .setDescription(`<a:dcworker:772431285460664330> Merhaba ! <@${message.author.id}> Şuan Ne Yazıkki Bakımdayız.`)
 .addField('Bakım Sebebi:', veri)
  .addField("Bu Sürede Bot Çalışmaya Bilir Aktif Olmayabilir Anlayışınız İçin Teşşekürler.")
 .setColor('#2f3136')
...
db.add(`komutklnm_${message.author.id}`, 1)
  ...
  if (y === 50) {
    ...
    .setDescription(`<a:ryzen_KirmiziYildiz:777951528388395039> Sunucu Puanı başarıyla **${y}** puana ulaştı! Premium mod aktif edildi!`)
...
(message.delete())    
  .then(message => message.delete(10000));
       ...
       if(db.has(`üyelikk_${message.member.id}`)) {
if(message.channel.id !== sunucu) return message.channel.send(`Kral, <@${message.author.id}> Burası komut kanalı değil. <#`+ sunucu+`> kanalına giderek komutlarımı ordan kullanabilirsin. `)
  .then
(message.delete())    
  .then(message => message.delete(10000));
    }
