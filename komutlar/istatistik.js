...
exports.run = async (bot, message, args) => {
  ...
  var osType = await os.type();
    ...
    cpuStat.usagePercent(function(err, percent, seconds) {
      ...
      setTimeout(() => {
        ...
        .addField(`<a:3111111111111:770616695139205171> ${dil.stat.pingshead}`, `${dil.stat.pings}`.replace("{ping1}", new Date().getTime() - message.createdTimestamp).replace("{ping2}", bot.ping), true)
        .addField(`<a:3111111111111:770616695139205171> ${dil.stat.uptimehead}`, `${duration}`, true)
        .addField(`<a:3111111111111:770616695139205171> ${dil.stat.data}`, stripIndents`
   **${dil.stat.voice}** ${bot.voiceConnections.size.toLocaleString()}
        ...
        **${dil.stat.channels}** ${bot.channels.size.toLocaleString()}
        `, true)
        .addField(`<a:3111111111111:770616695139205171> ${dil.stat.surums.version}`, stripIndents`
        **${dil.stat.surums.bot}** v${bot.ayarlar.versiyon}
        **${dil.stat.surums.discord}** v${Discord.version}
        **${dil.stat.surums.node}** ${process.version}
        `, true)
        .addField(`<a:3111111111111:770616695139205171> ${dil.stat.bellek}`, `${Math.round(process.memoryUsage().heapUsed / 1024 / 1024).toLocaleString()} MB`, true)
        .addField(`<a:3111111111111:770616695139205171> ${dil.stat.isletim}`, `${osType} ${osBit}`, true)
