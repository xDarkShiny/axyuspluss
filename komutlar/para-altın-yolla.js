...
exports.run = async (client, message, args) => {
  ...
  if (user.id === message.author.id) return message.reply(`${client.emojis.get('755433756759949352')} Kendine altınmı yollayacaksın ciddimisin?`);
...
if (isNaN(args[1])) return message.channel.send("Lütfen bir sayı gir.")
  ...
  if (altın < mesaj) {
    ...
    message.channel.send(`${client.emojis.get('762595446488170518')} Yeterince altının bulunmuyor, sende olan altın:  ${bulunanP === null  ? "0" : `${bulunanP}`}`)
} else if  (altın > mesaj) {

  const embed = new Discord.RichEmbed()
  .setColor('RANDOM')
  .setDescription(`${client.emojis.get('762595449180782603')} ${user} Adlı kullanıcıya altın yollandı, yollanılan miktar: ${mesaj}`)
