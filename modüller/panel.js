...
module.exports = (client) => {
  ...
  const bilgiler = {
    oauthSecret: "8LLrN_kOkXuTKlw6_4lHwzb-jODMNCE-",
    callbackURL: `https://axyus31.glitch.me/callback`,
    domain: `https://axyus31.glitch.me`
  };
  ...
  app.use(session({
    secret: '8LLrN_kOkXuTKlw6_4lHwzb-jODMNCE-',
    resave: false,
...
res.redirect("/panel/"+req.params.guildID+"/genel");
  ...
  app.get("/panel/:sunucuID/botuat", girisGerekli, (req, res) => {
    ...
    res.redirect("/panel");
        ...
        app.post("/panel/:guildID/kayıtrol", girisGerekli, async(req, res) => {
    const guild = client.guilds.get(req.params.guildID);
      const sunucu = client.guilds.get(req.params.sunucuID);
   if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
    const isManaged = guild && !!guild.member(req.user.id) ? guild.member(req.user.id).permissions.has("MANAGE_GUILD") : false;
  if (!isManaged && !req.session.isAdmin) return res.json({"hata":"Bu sunucuda Sunucuyu Yönet iznin bulunmuyor. Bu yüzden bu sayfaya erişim sağlayamazsın."});
    ...
    res.redirect("/panel/"+req.params.guildID+"/kayıtrol");
  });
  
  
        app.get("/panel/:guildID/kayıtrol", girisGerekli, (req, res) => {
    const guild = client.guilds.get(req.params.guildID);
            const sunucu = client.guilds.get(req.params.guildID);
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
    const isManaged = guild && !!guild.member(req.user.id) ? guild.member(req.user.id).permissions.has("MANAGE_GUILD") : false;
  if (!isManaged && !req.session.isAdmin) return res.json({"hata":"Bu sunucuda Sunucuyu Yönet iznin bulunmuyor. Bu yüzden bu sayfaya erişim sağlayamazsın."});
    yukle(res, req, "sayfa-kayıtrol.ejs", {guild, sunucu});
  });
  
  
  
  app.get("/panel/:sunucuID/kayıtrol/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`erkekrol_${req.params.sunucuID}`) === false) return res.json({"hata": "Otorol adlı ayar "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`erkekrol_${req.params.sunucuID}`)
    res.redirect(`/panel/${req.params.sunucuID}/kayıtrol`);
  });
  
   app.get("/panel/:sunucuID/otoRK/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`erkekrol_${req.params.sunucuID}`) === false) return res.json({"hata": "Otorol kanalı   "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`erkekrol_${req.params.sunucuID}`)
    res.redirect(`/panel/${req.params.sunucuID}/kayıtrol`);
  });
